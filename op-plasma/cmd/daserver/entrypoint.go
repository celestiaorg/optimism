...
func StartDAServer(cliCtx *cli.Context) error {
	...
	case cfg.CelestiaEnabled():
		...
		store := plasma.NewCelestiaStore(cfg.CelestiaConfig())
		server = plasma.NewCelestiaServer(cliCtx.String(ListenAddrFlagName), cliCtx.Int(PortFlagName), store, l)
	}
