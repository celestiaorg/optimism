...
L2OO_ADDRESS=0x70997970C51812dc3A010C7d01b50e0d17dc79C8
...
(
  cd ops-bedrock
  echo "Bringing up L1..."
  DOCKER_BUILDKIT=1 docker-compose build --progress plain -f docker-compose-testnet.yml
  docker-compose up -d l1 -f f docker-compose-testnet.yml
  wait_up $L1_URL
)
...
(
  cd ops-bedrock
  echo "Bringing up L2..."
  docker-compose up -d l2 -f docker-compose-testnet.yml
  wait_up $L2_URL
)
...
(
  ...
  L2OO_ADDRESS="$L2OO_ADDRESS" \
      docker-compose up -d op-proposer op-batcher -f docker-compose-testnet.yml

  echo "Bringing up stateviz webserver..."
  docker-compose up -d stateviz -f docker-compose-testnet.yml
)
