...
type BatcherService struct {
	...
	CelestiaIndexer      *indexer.BlockIndexer
}
...
func (bs *BatcherService) initDA(cfg *CLIConfig) error {
	idx, err := indexer.NewBlockIndexer("celestia.db")
	if err != nil {
		return err
	}
	client, err := celestia.NewDAClient(cfg.DaConfig.Rpc, cfg.DaConfig.AuthToken, cfg.DaConfig.Namespace, cfg.DaConfig.FallbackMode, cfg.DaConfig.GasPrice, idx)
	if err != nil {
		return err
	}
	bs.DAClient = client
